...
class OpcionArmaUnidad extends Component{
  ...
  render() {
    ...
    for(var i=0;i<cupos;i++){
      if(i<cuposOcupados){
        botonesArmas.push(<img key={i} onClick={this.quitaArma.bind(this)} className="img-circle img-responsive col-md-3 center-block"
          src={this.props.opArma.icono} alt={this.props.opArma.nombre} />
        )
      }else if(this.props.opArma.activo){
        botonesArmas.push(<img key={i} onClick={this.addArma.bind(this)} className="img-circle img-responsive disable-image col-md-3 center-block"
          src={this.props.opArma.icono} alt={this.props.opArma.nombre} />
        )
      }
    }
