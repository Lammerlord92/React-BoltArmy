...
class OpcionArma extends Component {
    ...
    render() {
        ...
        const addBotones = this.props.opciones.forEach(
            (value, index) => {
                var boton = < img id = { index }
                key = { this.props.opcion + value.nombre + index }
                onClick = { this.cambiaArmaSeleccionada.bind(this) }
                className = "img-circle img-responsive disable-image col-md-3 center-block"
                src = { value.icono }
                alt = { value.nombre }
                />;
                if (index === this.props.opcionEscogida) {
                    boton = < img key = { this.props.opcion + value.nombre + index }
                    className = "img-circle img-responsive col-md-3 center-block"
                    src = { value.icono }
                    alt = { value.nombre }
                    />;
        ...
        return ( 
              <div className = "col-sm-12 text-center">
                ...
                <div className = "col-sm-12 text-center"> { botonesArmas } </div> 
              </div > 
        )
    }
}
